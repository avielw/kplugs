...
class PlugWorld(object):
	...
	def _flush(self):
		...
		ret = ""
		for i in xrange(len(self._cmds)):
	...
	def ioctl(self, op, data, complete = False):
		if self.remote:
			...
			arr = array.array('B', [ord(i) for i in data])
	...
	def alloc(self, size):
		if self.remote:
			cache = filter(lambda i:self._free_cache[i] >= size, self._free_cache)
			if len(cache):
				if len(cache) == 1:
	...
	def free(self, addr, force=False):
		if self.remote:
			if type(addr) == list:
				...
				obj = filter(lambda i:i[0] == addr, self.my_objects)[0]
			...
			obj = filter(lambda i:ctypes.addressof(i) == addr, self.my_objects)[0]
			self.my_objects.remove(obj)
